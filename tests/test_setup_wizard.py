...
def _docs_manifest_block() -> str:
    ...
    # The heading is `## 2. Manifest`; the YAML block can appear after
    # any amount of intervening prose (so the doc can include user-facing
    # context before the paste-in block).
    match = re.search(
        r"## 2\. Manifest\n.*?```yaml\n(?P<manifest>.*?)\n```",
