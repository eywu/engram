...
def test_check_launchd_bridge_job_not_installed_fails() -> None:
    check = check_launchd_job(
        "launchd_bridge",
        "launchd bridge job",
        "com.engram.bridge",
        launchctl_list=lambda: "PID\tStatus\tLabel\n123\t0\tcom.engram.v3.nightly\n",
    )
...
def test_check_launchd_nightly_job_not_installed_warns_when_optional() -> None:
    check = check_launchd_job(
        "launchd_nightly",
        "launchd nightly job",
...
def test_doctor_cli_json_warns_when_nightly_launchd_job_is_missing(
    tmp_path: Path,
    clean_env: None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
